...
import { courses } from "../../Redux/Features/CourseSlice";

const Course = () => {
  const dispatch = useDispatch();
  const allCoursesList = useSelector((state: any) => state.course.courses);
    ...
    <div>
      ...
      <section className="px-10 py-20 bg-[#f6f6f6]">
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
          {allCoursesList?.map((item: any, index: number) => (
            <Link
              to={`/courses/${item._id}`}
