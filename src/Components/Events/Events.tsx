...
import { AddEvents } from "../../Redux/Features/EventSlice";
import { useNavigate } from "react-router-dom";

const Events = () => {
  const dispatch = useDispatch();
  const navigate = useNavigate();
  const eventArray = useSelector((state: any) => state.event.events);
  ...
  const toDetailComponent = (_id: string, item: any) => {
            ...
            >
              <div className="flex">
                ...
                <div className="grid ml-5">
                  <p
                    onClick={() => toDetailComponent(item._id, item)}
